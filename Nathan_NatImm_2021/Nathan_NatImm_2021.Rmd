...
plot_grid(plotlist = list(fig.umap, fig.voom, fig.vp, fig.pct), nrow=2, labels=LETTERS[1:4], align="hv", axis="t")
...
```{r plot.pi}
library(qvalue)
topTable(res.dl, coef = 'TB_statusCASE', number=Inf) %>%
  as_tibble %>% 
  group_by(assay) %>%
...
vp_res = fitExtractVarPartModel(cobj, ~ (1|TB_status) + (1|donor) + (1|batch) + (1|season) + prop_NAT, colData(pb))
...
dcmp$x %>%
  ...
  as.data.frame %>%
  ggplot(aes(PC1, PC2, color=TB_status)) +
    geom_point()
...
Show
 - batch correlates with GC content
 - Donor correlations with read count per gene
