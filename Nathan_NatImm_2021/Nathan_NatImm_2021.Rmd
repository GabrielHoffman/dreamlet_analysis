...
fig.heat1 = plotGeneHeatmap( res.dl, coef="TB_statusCASE", genes=geneIds(go.gs[[gs1]]), assays=CT, zmax=zmax, transpose=TRUE) + 
...
gs2 = 'ER541_cellular_response_to_interferon-gamma_(GO0071346)'
fig.heat2 = plotGeneHeatmap( res.dl, coef="TB_statusCASE", genes=geneIds(go.gs[[gs2]]), assays=CT, zmax=zmax, transpose=TRUE) + 
  ggtitle(trimGenesets(gs2)) + 
  theme(legend.key.size = unit(.3, 'cm'),
    axis.text.x=element_text(size=5, angle=20),
    axis.text.y=element_text(size=5))+ 
  coord_flip()

plot_grid(fig.heat1, fig.heat2, nrow=1)
```

f( res.dl, coef="TB_statusCASE", genes=geneIds(go.gs[[gs1]]), assays=CT, zmax=zmax)
