...
# Dreamlet analysis of single cell data
...
# number of batches
df_properties$n_batches = nlevels(sce$batch)

# number of donors
df_properties$n_donors = nlevels(sce$donor)

# number of donors
df = unique(data.frame(donor = sce$donor, TB_status = sce$TB_status))
df_properties$n_cases = sum(df$TB_status == "CASE")
df_properties$n_controls = sum(df$TB_status == "CONTROL")

df_properties$n_cells = ncol(sce)
...
# variance partitioning analysis
form = ~ TB_status + (1|donor) + (1|batch) + (1|season) + (1|sex) + age + I(age^2) + prop
