...
# https://hoffmg01.dmz.hpc.mssm.edu/dreamlet_analysis/Mathys_2023/Mathys_2023_Major_Cell_Type.html
...
tab.limma = lapply(assayNames(pb), function(id){

  # remove SubID with insufficient read count
  lib.size = colSums(assay(pb, id))
  pb_tmp = pb[,lib.size > 1000]
  pb_tmp = pb_tmp[,pb_tmp$sex != "NA"]
  pb_tmp$sex = factor(pb_tmp$sex)
  pb_tmp = pb_tmp[,!is.nan(pb_tmp$pmi)]
  ...
  fit = eBayes(fit)
  tab = topTable(fit, coef="ADdiag2typesAD", number=Inf)
  data.frame(assay = id, ID = rownames(tab), tab)
  })
tab.limma = do.call(rbind, tab.limma)

write_parquet(tab.limma, "Mathys_Cell_2023_Major_Cell_Type_limma.parquet")
```
...
tab.deseq2 = lapply(assayNames(pb), function(id){
  ...
  pb_tmp$age = scale(pb_tmp$age)
  pb_tmp$pmi = scale(pb_tmp$pmi)

  counts = assay(pb_tmp, id)

  # for cell type Vas, use poscounts 
  # otherwise DESeq crashes
  sfType = ifelse(id == "Vas", "poscounts", "ratio")

  dds <- DESeqDataSetFromMatrix(counts, colData(pb_tmp), form)
  dds <- DESeq(dds, sfType=sfType)
  res <- results(dds)
  data.frame(assay = id, ID = rownames(res), res)
  })
tab.deseq2 = do.call(rbind, tab.deseq2) 

write_parquet(tab.deseq2, "Mathys_Cell_2023_Major_Cell_Type_DESeq2.parquet")
```
