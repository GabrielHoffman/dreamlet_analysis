...
system("git pull"); rmarkdown::render("Mathys_2023_bulk.Rmd");


# https://hoffmg01.u.hpc.mssm.edu/dreamlet_analysis/Mathys_2023/Mathys_2023_Cell_Type.html
...
# UMAP embedding
```{r embedding}
plotProjection(sce, "X_umap", annotation='bulk') 
```
...
```{r voom.plot, fig.height=25, fig.width=12}
...
```{r vp.plot, fig.height=25, fig.width=12}
# Summarize variance fractions genome-wide for each cell type
plotVarPart(sortCols(vp.lst), label.angle=60, ncol=5)   
```
...
# Volcano
```{r volcano, fig.height=25, fig.width=12}
plotVolcano(fit, "ADdiag2typesAD", ncol=5)
```

```{r write.file}
topTable(fit, coef = "ADdiag2typesAD", number=Inf) %>%
  write.table(file="Mathys_2023_bulk_ADdiag2typesAD.tsv", quote=FALSE, sep="\t", row.names=FALSE)
```


## Highlight genes
```{r genehatmap, fig.height=12, fig.width=15, cache=FALSE}
...
```{r plotForest, fig.height=25, fig.width=16, cache=FALSE}
...
```{r plot.pi, fig.height=7, fig.width=12}
...
df = colData(sce) %>%
  xtabs( ~ Individual + Cell_Type,.) %>%
  as_tibble %>%
  pivot_longer(cols=Individual) %>%
  mutate(assay = Cell_Type) %>%
  group_by(assay) %>%
...
```{r zenith.heatmap.1, fig.width=12, fig.height=50, cache=FALSE} 
...
```{r zenith.heatmap.2, fig.width=16, fig.height=50, cache=FALSE} 
...
saveRDS(fitList, file="Mathys_Cell_2023_Cell_Type_dreamlet_fit.RDS")
df_all = lapply(names(fitList), function(trait){
  ...
  if( length(coefSet) == 1){    
    cols = c("assay", "ID", "AveExpr", "t", "P.Value", "adj.P.Val", "trait", "coefSet")
    tab = tab %>% 
      select(all_of(cols)) %>%
      rename(stat = "t")
  }else{
    cols = c("assay", "ID", "AveExpr", "F", "P.Value", "adj.P.Val","trait", "coefSet")
    tab = tab %>% 
      select(all_of(cols)) %>%
      rename(stat = "F")
  }
...
write_parquet(df_all, "Mathys_Cell_2023_Cell_Type_dreamlet.parquet")
```
