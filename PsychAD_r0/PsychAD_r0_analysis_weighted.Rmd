...
file = "./topTable_PsychAD_r0_weighted.tsv"
...
fit_n.cells = lm(nDE ~ log10(sum.cells/n_retained), df3)
pv = coef(summary(fit_n.cells))[2,4]
pv = format(pv, digits=2)

fig.nDE_ncells = ggplot(df3, aes(sum.cells/n_retained, nDE, color=assay, size=n_retained, label=assay)) +
  geom_abline(intercept=coef(fit_n.cells)[1], slope=coef(fit_n.cells)[2], color="grey30") +
  geom_point() +
  scale_x_log10() +
  theme_classic() +
