...
fig_batch_GC_mod = fig_batch_GC + 
  ...
  coord_flip(ylim=c(0,0.155), expand=FALSE) +
  ylab('Corr of batch effect\nand GC content') +
  xlab('')

figF = ggplot() + annotate("text", x=0, y=0, label="Differ, expressed genes") + theme_void()

fig = plot_grid(fig_UMAP, fig_mv, fig_vp, fig.percent + theme(legend.position="bottom"), fig.cells_subject, fig_batch_GC_mod, figF, figG, labels=LETTERS, ncol=4, axis="tblr" , align="hv")
...
fig.forest = plotForest( fit, coef = 'DxAD', gene = 'PTPRG', assays=ctorder) + theme(aspect.ratio=1, legend.position="none")


fig = plot_grid(fig.volcano, fig.gene_heatmap + theme(legend.position="bottom"), fig.forest, figList[['PTPRG']],
