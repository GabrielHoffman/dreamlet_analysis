...
geneExpr = res.proc[["Micro_PVM"]]

W = t(geneExpr[c("XIST"),]$weights)
range(W / min(W))


form = ~ (1|SubID) + (1|poolID) + (1|Sex) + (1|Dx) + scale(Age)
# form = ~ (1|SubID) + (1|Sex) + (1|Dx) 

data = colData(res.proc)[colnames(geneExpr),]
# data$poolID = gsub("-([a-zA-Z]+\\d)$", "", data$poolID)


df = fitExtractVarPartModel(geneExpr[genes,], form, data)

plotPercentBars(sortCols(df))
...
file = '~/www/test.pdf'
ggsave(file=file, fig, width=12, height=7)
