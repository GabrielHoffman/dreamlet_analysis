...
# remove set with > 1000 genes
go.gs.en = go.gs.en[sapply(go.gs.en, function(x)length(geneIds(x))) < 1000]
...
```{r plot.zenith2, fig.width=15, fig.height=16, cache=FALSE}
...
# # Gene set analysis using zenith
# ```{r zenith2}
# go.BioCarta = getGenesets( org = "hsa", 
#                   db = "enrichr", 
#                   lib = "BioCarta_2016",
#                   gene.id.type = "SYMBOL", 
#                   return.type = "GeneSetCollection")

# res.zenith2 = zenith_gsa( fit, go.BioCarta, 
#               coef = 'AD1')
# ```
...
```{r mashr.zenith}
df_gs = zenith_gsa(res_mash, go.gs.en)
```

```{r mashr.zenith.plot, fig.width=15, fig.height=16}
