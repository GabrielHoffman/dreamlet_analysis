...
```{r frac.de2}
df_de %>%
  mutate(frac.DE = nDE / nGenes) %>%
  mutate(frac.null = 1 - frac.DE) %>%
  select(assay, frac.DE, frac.null, nGenes) %>%
  pivot_longer(cols=c('frac.DE', 'frac.null'), values_to='fraction') %>%
  ggplot(aes(x=nGenes/2, y=fraction, group=name, fill=name, width=nGenes)) +
    geom_bar(stat = "identity") +
    coord_polar("y", start=0) + 
    facet_wrap(.~ assay) + theme_void() +
...
fig.vp_ncells = ggplot(df3, aes(sum.cells/n_retained, mean.vp, color=assay, size=n_retained, label=assay)) +
  ...
  geom_text_repel(size=3, box.padding =.5, min.segment.length=1, max.overlaps = 15) +guides(color = "none", size="none") +
