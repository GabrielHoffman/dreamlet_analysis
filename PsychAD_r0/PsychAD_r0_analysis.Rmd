...
fig.percent = plotPercentBars(df.vp.sub, assays=ctorder) + theme(aspect.ratio=1)
fig.percent
```
...
```{r frac.de}
ggplot(df_de, aes(2, assay)) +
        geom_tile(fill="white") +
        geom_point(aes(color=nDE, size=nDE/nGenes)) +
        scale_color_gradient(name = "# genes\n< 5% FDR", low = "blue",  high = "red", lim=c(0, NA)) +
        scale_size_continuous(name = "Fraction of genes\n< 5% FDR") +
        theme_classic() +
        theme(aspect.ratio=nrow(df_de), axis.text.x=element_blank(), axis.ticks.x=element_blank()) +
        xlab('') + ylab('') 
```
...
fit_n.cells = lm(mean.vp ~ log10(sum.cells/n_retained), df3, weights=df3$n_retained)
# summary(fit)
pv = coef(summary(fit_n.cells))[2,4]
pv = format(pv, digits=2)

fig.vp_ncells = ggplot(df3, aes(sum.cells/n_retained, mean.vp, color=assay, size=n_retained, label=assay)) +
  geom_abline(intercept=coef(fit)[1], slope=coef(fit)[2], color="grey30") +
  geom_point() +
  scale_x_log10() +
  theme_classic() +
  theme(aspect.ratio=1) + 
  scale_size(name="# Subjects") +
  xlab("Mean # cells per Subject") +
  ylab("Mean % variance across Subject") +
  geom_text_repel(size=4, box.padding =.5, min.segment.length=1) +guides(color = "none", size="none") +
...
figF = ggplot() + annotate("text", x=0, y=0, label="Differ, expressed genes") + theme_void()
figG = ggplot() + annotate("text", x=0, y=0, label="Differ, expressed genes") + theme_void()

# fig.cells_subject
fig = plot_grid(fig_UMAP, fig_mv, fig_vp, fig.percent + theme(legend.position="bottom"), fig_batch_GC_mod, fig.vp_ncells, figG, labels=LETTERS, ncol=4, axis="tblr" , align="hv")
...
fig.PDE10A = plotForest( fit, coef = 'DxAD', gene = 'PDE10A', assays=ctorder) + theme(aspect.ratio=1, legend.position="none")
