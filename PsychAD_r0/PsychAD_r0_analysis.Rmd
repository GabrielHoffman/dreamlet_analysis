...
Public release 0 includes `r length(table(sce$Channel))` samples, `r length(table(sce$round_num))` rounds, `r length(table(sce$batch))` [10X](https://www.10xgenomics.com/products/single-cell-gene-expression) batches, `r length(table(sce$SubID))` donors and `r format(ncol(sce), big.mark=',')` cells passing QC.
...
```{r umap6, dev="png", fig.height=12, fig.width=12}
df %>%
filter(celltype9 == 'Oligodendrocyte') %>%
mutate(anno = droplevels(anno)) %>%
ggplot(aes(V1, V2, color=anno)) + geom_scattermore() + theme_classic() + theme(aspect.ratio=1) + guides(colour = guide_legend(override.aes = list(size = 1.5))) + xlab("UMAP1") + ylab("UMAP2")
```



```{r exit333, cache=FALSE}
knitr::knit_exit()
```
...
```{r mashr.zenith.plot, fig.width=15, fig.height=16, cache=FALSE}
df_gs$assay = factor(df_gs$assay, names(fit))
plotZenithResults(df_gs, 5, 1) 
```

# test
```{r test}
plotForest(res_mash, "PTPRG") 
plotForest(res_mash, "SPRY4-AS1") 
plotForest(res_mash, "SPRY4") 
plotForest(res_mash, "PDE10A") 



get_lfsr(res_mash$model)['SPRY4',]

df = extractData(res.proc, "Excitatory neuron")

genes = c("SPRY4-AS1", "SPRY4", "FGF1")
# df[,genes]


df_controls = df[df$AD == 0,]

df_cor = t(cor(df_controls[,"SPRY4-AS1",drop=FALSE], df_controls[,-c(1:8)]))
df_cor = df_cor[order(abs(df_cor), decreasing=TRUE),,drop=FALSE]

head(df_cor, 10)

cutoff = .6
ngenes = sum(abs(df_cor) >= cutoff) - 1

df_cor %>%  
  as.data.frame %>%
  rownames_to_column %>%
  rename(gene = rowname , correlation = `SPRY4-AS1`) %>% 
  filter(correlation >= cutoff) %>%
  filter(gene != "SPRY4-AS1") %>%
  mutate(gene = factor(gene, rev(gene))) %>%
  ggplot(aes(x=1, y=gene, fill = correlation)) +
  geom_tile() +
  theme_classic() +
  theme(aspect.ratio=ngenes, 
