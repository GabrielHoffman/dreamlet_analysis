...
getCellCounts = function(sce, column1, column2){

  df = colData(sce) %>%
    as_tibble %>%
    group_by(across(column1), across(column2)) %>%
    summarize( count=n()) %>%
    distinct 

  M = sparseMatrix( as.numeric(df[[column1]]), 
            as.numeric(df[[column2]]), 
            x=df$count,
            dims=c(nlevels(df[[column1]]), nlevels(df[[column2]])))
  rownames(M) = levels(df[[column1]])
  colnames(M) = levels(df[[column2]])
...
hcl = buildClusterTree(sce, "X_pca_regressed_harmony", "celltype")

# counts = cellCounts(pb)

counts = getCellCounts(sce, "celltype", "Channel")
...
topTable(fit, "AD1", number=Inf)

res = treeTest( fit, cobj, hcl, coef="AD1", method="FE")
plotTreeTest(res) + xlim(0, 11)

res = treeTest( fit, cobj, hcl, coef="AD1", method="RE2C")
plotTreeTest(res) + xlim(0, 11)
```
