...
i = grep("Mono", res.gsa$assay)

plotZenithResults(res.gsa[i,], 5, 3)
```

```{r heatmap, eval=FALSE}

pfx = "GO0034340"
grep(pfx, names(go.gs), value=T)
...
ggsave(fig, file="~/www/test.pdf", height=20, width=12)
...
# Figures
```{r figures, fig.height=8, fig.width=12, eval=FALSE}
library(paletteer)
...
# form = ~ Age + (1|sex) + (1|Source)
form = ~1
res.proc2 = processAssays(pb, form,  
                          # min.count = 3,
                          # min.cells = 10,
                          BPPARAM = SnowParam(6), 
                          assay="cMono" )
...
colData(res.proc)$isCOVID = grepl("^COVID", colData(res.proc)$Source)

form = ~ Age + (1|sex) + (1|Source) + (1|Institute)
res.vp2 = fitVarPart(res.proc2, form, BPPARAM=SnowParam(12), assay="cMono" )


df = inner_join(res.vp2 %>% 
                  as_tibble %>%
                  mutate(ID = gene), 
                topTable(fit, coef="sexmale", number=Inf) %>% 
                as_tibble %>%
                filter(assay == "cMono"),
                by = 'ID')

incl = res.vp$assay == "cMono"
figC = plotVarPart( sortCols(res.vp[incl,]) )

figD = plotVolcano(fit[["cMono"]], coef="SourceCOVID_MILD")

fig = plot_grid(figA, figB, figC, figD, ncol=4, labels=LETTERS)
ggsave(file="~/www/test.pdf", fig, height=8, width=12)



with(colData(pb), xtabs(~Source + Institute))
